...
SELECT SUM(DISTINCT do.TOTAL_AMOUNT) AS TOTAL_PEDIDOS_FATO 
FROM FACT_SALES f JOIN DIM_ORDER do ON do.ORDER_SK = f.ORDER_SK;

-- Reconciliação de impostos
SELECT SUM(TAX_AMOUNT) AS IMPOSTOS_STAGING FROM STAGING.ORDERS;
SELECT SUM(DISTINCT do.TAX_AMOUNT) AS IMPOSTOS_FATO 
FROM FACT_SALES f JOIN DIM_ORDER do ON do.ORDER_SK = f.ORDER_SK;
